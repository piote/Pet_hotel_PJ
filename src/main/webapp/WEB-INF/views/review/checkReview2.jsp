                ...
                <c:when test="${reviewList != null && user_id == 'admin'}">
                    <c:forEach var="review" items="${reviewList }" varStatus="reviewNum">
                        ...
                        <tr align="center">
                            ...
                            <td width="7%"> <a class='cls1'
                                    href="${contextPath}/review/viewReview.do?reviewNO=${review.reviewNO}">수정</a> </td>
                            <td width="7%"> <input type=button value="삭제"
                                    onClick="fn_remove_review('${contextPath}/review/removeReview.do', ${review.reviewNO})">
                            </td>
                        </tr>
                ...
                <c:when test="${totReview >100 }">
                    ...
                    <c:forEach var="page" begin="1" end="10" step="1">
                        <c:if test="${section >1 && page==1 }">
                            <a class="no-uline"
                                href="${contextPath }/review/checkReview2.do?section=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp;
                                pre </a>
                        </c:if>
                        <a class="no-uline"
                            href="${contextPath }/review/checkReview2.do?section=${section}&pageNum=${page}">${(section-1)*10
                            +page } </a>
                        <c:if test="${page ==10 }">
                            <a class="no-uline"
                                href="${contextPath }/review/checkReview2.do?section=${section+1}&pageNum=${section*10+1}">&nbsp;
                                next</a>
                        </c:if>
                    </c:forEach>
                </c:when>
                    ...
                    <c:forEach var="page" begin="1" end="${totReview/10 +1}" step="1">
                        <c:choose>
                            <c:when test="${page==pageNum }">
                                <a class="sel-page"
                                    href="${contextPath }/review/checkReview2.do?section=${section}&pageNum=${page}">${page
                                    } </a>
                            </c:when>
                            <c:otherwise>
                                <a class="no-uline"
